...
export function SpotlightSection({ competitions, now, onOpenDetail }: SpotlightSectionProps) {
            ...
            <div className="grid gap-4 p-4 md:p-[1.125rem] lg:grid-cols-[1.1fr_0.9fr] lg:p-5">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-300 sm:text-base">
                    {primaryCompetition.description}
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-2.5 self-start rounded-[1rem] border border-white/7 bg-black/8 p-3.5">
                <div className="border-b border-white/6 pb-3">
                  <p className="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Deadline pendaftaran</p>
                  <p className="mt-2 text-xl font-semibold text-zinc-100">
                    {formatDate(primaryCompetition.regEnd)}
                  </p>
                </div>
                <div className="border-b border-white/6 pb-3">
                  <p className="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Penyisihan</p>
                ...
                <div>
                  <p className="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Sisa waktu</p>
                  <p className="mt-2 text-3xl font-semibold text-accent-gold/95">
                    {primaryDaysLeft >= 0 ? `${primaryDaysLeft} hari` : "Terlambat"}
                  </p>
                  ...
                  >
                    ...
                    <div className="mt-3 grid gap-2.5 border-t border-white/6 pt-3 text-sm text-zinc-300">
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-xs uppercase tracking-[0.2em] text-zinc-500">Deadline pendaftaran</span>
                        <span className="font-medium text-zinc-100">{formatDate(competition.regEnd)}</span>
                      </div>
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-xs uppercase tracking-[0.2em] text-zinc-500">Sisa waktu</span>
                        <span>{daysLeft >= 0 ? `${daysLeft} hari lagi` : "Sudah lewat"}</span>
                      </div>
                    </div>
                  </button>
                </li>
